...
for i, psnr_path in enumerate(psnr_paths):
	...
	for j, key in enumerate(dataset_keys):
		...
		print ("x2_np.mean:",np.mean(psnr_np_2, axis=0).tolist())
		print ("x3_np.mean:",np.mean(psnr_np_3, axis=0).tolist())
		print ("x4_np.mean:",np.mean(psnr_np_4, axis=0).tolist())
...
for i, key in enumerate(keys):
	...
	for epoch in epochs:
		print ("epoch=",epoch)
		print ("psnr_dict[epoch]=",psnr_dict[epoch])
	...
	plt.figure(i)
	print ("key=",key)
	print ("len(x_axis), len(bicub_mean), len(vdsr_mean_2) ",len(x_axis), len(bicub_mean), len(vdsr_mean_2))
	print ("vdsr_mean_2=",vdsr_mean_2)
	print ("vdsr_mean_3=", vdsr_mean_3)
	print ("vdsr_mean_4=", vdsr_mean_4)
	print ("x2_np.argmax_np.max", np.argmax(vdsr_mean_2), np.max(vdsr_mean_2))
	print ("x3_np.argmax_np.max", np.argmax(vdsr_mean_3), np.max(vdsr_mean_3))
	print ("x4_np.argmax_np.max", np.argmax(vdsr_mean_4), np.max(vdsr_mean_4))
